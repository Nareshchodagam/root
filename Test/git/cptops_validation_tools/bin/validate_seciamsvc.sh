...
# iamportal is not running everywhere. Check deployment status and only add to service list if installed
if systemctl list-unit-files --full --all | grep -Fq iamportal.servce; then
  services+=(iamportal.service)
fi
