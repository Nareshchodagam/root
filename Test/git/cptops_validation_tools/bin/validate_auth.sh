...
MASTER_FILE=/root/KRBMASTER
if test -f "$MASTER_FILE"; then
  echo -e "\nHost is the current master for its realm. Will check kadmin service and skip kprop.service"
  services+=( kadmin.service )
else
  echo -e "\nHost is not the current master for its realm. Skipping kadmin.service and checking kprop.service"
  services+=( kprop.service )
fi
