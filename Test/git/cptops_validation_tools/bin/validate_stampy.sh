...
validateStampy() {
#
# For the stampy role we need to check the file signing functionality
# This will attempt to create a file in /tmp, sign it and remove it
#
  touch /tmp/stampy-test.txt
  if stampyctl sign files --format=cms /tmp/stampy-test.txt; then
    echo -e "\nSigning test successful. \e[32mPatching can continue\e[0m"
    rm /tmp/stampy-test.txt
  else
    echo -e "\n\e[31mError\e[0m: File signing not healthy. Please contact the Stampy team"
    rm /tmp/stampy-test.txt
    exit 1
  fi
}
...
#
  if systemctl list-unit-files --full --all | grep -Fq ${1}; then
    ...
    if systemctl is-active --quiet ${1}; then
      ...
      if stampyctl cluster; then
        echo -e "Cluster is healthy. Checking signing functionality...\n"
        validateStampy
      else
