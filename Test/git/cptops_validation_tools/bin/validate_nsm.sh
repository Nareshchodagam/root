...
# For nsm hosts, either bro or suricata should be running. Not both
services=(
  bro.service
  suricata.service
)
...
#
  # Do not attempt to auto recover services not running prior to patching
  if [ "$STAGE" == 'post-patch' ] && [ ! -f "/root/${1}.txt" ]; then
    echo -e "\n${1} was not running prior to patching. \e[32mPatching can continue\e[0m"
    return 0
  fi
  ...
  if systemctl list-unit-files --full --all | grep -Fq ${1}; then
    ...
    if systemctl is-active --quiet ${1}; then
      ...
      if [ "$STAGE" == 'pre-patch' ]; then
        # Take note of the running service for post-patch checks to prevent autorecovery running on wrong service
        touch /root/${1}.txt
      else
        rm /root/${1}.txt
      fi
...
runChecks() {
  ...
  if [ ${#pre_patch_errors[@]} -gt 1 ]; then
    ...
    exit 1
  elif [ ${#post_patch_errors[@]} -gt 1 ]; then
